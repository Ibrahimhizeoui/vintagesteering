...
if (function_exists('add_theme_support')) {
    ...
    add_image_size('280x230', '280', '230', array('center', 'center'));
...
if (!function_exists('imic_excerpt')) {
    function imic_excerpt($limit = 50, $postID = null)
    {
        return wp_trim_words(get_the_excerpt($postID), $limit, imic_custom_read_more());
    }
}
/* 	Comment Styling
  /*----------------------------------------------------------------------------------- */
    ...
    if(!empty($specs))
	{
		...
		if($int==0||$int==2)
		{
			...
			else
			{
				...
				$intField = get_post_meta($id, 'char_'.$price_id_slug, true);
                $char_price = number_format($intField);
				if($label_position==0)
				{
		...
		else
		{
			...
			$intField = (int) get_post_meta($id,'int_'.$slug, true);
            $int_value = number_format($intField);
			if($int_value != '')
			{
				if($label_position==0)
				{
...
function imic_vehicle_title($id, $specs, $this_specification)
{
    ...
    /**
     * Assumption is that 'imic_specifications_upd_st' is set somewhere in the theme files,
     * however, not through the generic means, so set by setting a default value, which in this case is (1)
     *
     * Below they are checking for if this is 0 not 1, and everytime a product is created it
     * sets that title to "listing", further down this function.
     *
     * So for now we are temporarily removing this
     * ==================
     * $data_type = (get_option('imic_specifications_upd_st') != 1) ? 0 : get_option('imic_specifications_upd_st');
     */
	...
	if(!empty($specs))
    {
		foreach($specs as $highlight)
		{
            ...
            $value_existence = imic_value_search_multi_array($highlight, $this_specification);
