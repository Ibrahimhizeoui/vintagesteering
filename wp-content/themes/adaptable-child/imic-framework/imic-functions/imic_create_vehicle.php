...
if ( !function_exists( 'imic_create_vehicle' ) )
{
    function imic_create_vehicle( )
    {
        ...
        $adminEmailSent = get_post_meta( $post_id, 'imic_plugin_admin_email_sent', true );

        if($step > 3 && $adminEmailSent != 'true')
        {
            update_post_meta( $post_id, 'imic_plugin_admin_email_sent', 'true' );
            send_new_listing_email($post_id);
        }
    ...
    function send_new_listing_email($post_id)
    {
        $adminEmail = get_option('admin_email');
        $post = get_post($post_id);

        $message = 'A new listing has been submitted for approval: <a href="http://vintagesteering.provideit.se/wp-admin/post.php?action=edit&post=' . $post_id .'">http://vintagesteering.provideit.se/wp-admin/post.php?action=edit&post=' . $post_id .'</a>';

        $sent = vintage_send_mail($adminEmail, 'New listing - ' . $post->post_title, "Contact Admin", ["%content%" => $message]);
    }
}
